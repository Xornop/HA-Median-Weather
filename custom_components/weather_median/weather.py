...
class WeatherMedianEntity(WeatherEntity):
    ...
    async def _async_update_forecasts(self) -> None:
        supports_daily, supports_hourly = [], []
        for source in self._sources:
            if (state := self.hass.states.get(source)):
                feat = state.attributes.get("supported_features", 0)
                if feat & WeatherEntityFeature.FORECAST_DAILY: supports_daily.append(source)
                if feat & WeatherEntityFeature.FORECAST_HOURLY: supports_hourly.append(source)
        ...
        if supports_daily: feat |= WeatherEntityFeature.FORECAST_DAILY
        if supports_hourly: feat |= WeatherEntityFeature.FORECAST_HOURLY
    ...
    def _converted_attr(self, attr: str, unit_attr: str, conv: Any) -> list[float]:
        return [c for s in self._get_source_states() if (r := _to_float(s.attributes.get(attr))) is not None and (c := conv(r, s.attributes.get(unit_attr))) is not None]

    @property
    def native_temperature(self) -> float | None: return _median(self._converted_attr("temperature", ATTR_TEMPERATURE_UNIT, self._convert_temperature))
    @property
    def native_apparent_temperature(self) -> float | None: return _median(self._converted_attr("apparent_temperature", ATTR_TEMPERATURE_UNIT, self._convert_temperature))
    @property
    def humidity(self) -> float | None: return _median([v for s in self._get_source_states() if (v := _to_float(s.attributes.get("humidity"))) is not None])
    @property
    def native_wind_speed(self) -> float | None: return _median(self._converted_attr("wind_speed", ATTR_WIND_SPEED_UNIT, self._convert_speed))
    @property
    def wind_bearing(self) -> float | None: return _circular_median([v for s in self._get_source_states() if (v := _to_float(s.attributes.get("wind_bearing"))) is not None])
    @property
    def native_wind_gust_speed(self) -> float | None: return _median(self._converted_attr("wind_gust_speed", ATTR_WIND_SPEED_UNIT, self._convert_speed))
    @property
    def native_pressure(self) -> float | None: return _median(self._converted_attr("pressure", ATTR_PRESSURE_UNIT, self._convert_pressure))
    @property
    def native_visibility(self) -> float | None: return _median(self._converted_attr("visibility", ATTR_VISIBILITY_UNIT, self._convert_distance))
    @property
    def uv_index(self) -> float | None: return _median([v for s in self._get_source_states() if (v := _to_float(s.attributes.get("uv_index"))) is not None])
    @property
    def dew_point(self) -> float | None: return _median(self._converted_attr("dew_point", ATTR_TEMPERATURE_UNIT, self._convert_temperature))
    @property
    ...
    def _build_median_forecast(self, f_type: str) -> list[Forecast]:
        available = [(src, self._forecast_cache[(src, f_type)]["data"], self._forecast_cache[(src, f_type)]["units"]) for src in self._sources if (src, f_type) in self._forecast_cache]
        if not available: return []
        lead = available[0][1]
        res = []
        for l_slot in lead:
            if not (dt := l_slot.get(ATTR_FORECAST_TIME)): continue
            ...
            for src, slots, units in available:
                if (slot := next((s for s in slots if s.get(ATTR_FORECAST_TIME) == dt), None)):
                    ...
                    if (v := _to_float(slot.get(ATTR_FORECAST_WIND_BEARING))) is not None: bearings.append(v)
                    if (v := _to_float(slot.get(ATTR_FORECAST_PRECIPITATION))) is not None and (c := self._convert_precipitation(v, units.get(ATTR_PRECIPITATION_UNIT))) is not None: precips.append(c)
                    if (v := _to_float(slot.get(ATTR_FORECAST_HUMIDITY))) is not None: humids.append(v)
                    if (v := slot.get(ATTR_FORECAST_CONDITION)): conds.append(str(v))
            
            ent: Forecast = {ATTR_FORECAST_TIME: dt}
            if (v := _median(temps)) is not None: ent[ATTR_FORECAST_TEMP] = round(v, 1)
            if (v := _median(templows)) is not None: ent[ATTR_FORECAST_TEMP_LOW] = round(v, 1)
            if (v := _median(winds)) is not None: ent[ATTR_FORECAST_WIND_SPEED] = round(v, 1)
            if (v := _circular_median(bearings)) is not None: ent[ATTR_FORECAST_WIND_BEARING] = round(v, 1)
            if (v := _median(precips)) is not None: ent[ATTR_FORECAST_PRECIPITATION] = round(v, 2)
            if (v := _median(humids)) is not None: ent[ATTR_FORECAST_HUMIDITY] = round(v, 1)
            if (v := _majority_vote(conds)) is not None: ent[ATTR_FORECAST_CONDITION] = v
            res.append(ent)
        return res
