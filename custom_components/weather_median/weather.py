...
class WeatherMedianEntity(WeatherEntity):
    ...
    def _temperature_from_sources(self, attribute: str) -> list[float]:
        """Collect temperatures converted to this entity's native unit."""
        values = []
        for state in self._get_source_states():
            val = state.attributes.get(attribute)
            if val is None:
            ...
            try:
                ...
                continue
            unit = state.attributes.get("temperature_unit", self.native_temperature_unit)
            try:
                val = TemperatureConverter.convert(val, unit, self.native_temperature_unit)
            except Exception:
                pass
            values.append(val)
        return values
    ...
    def native_temperature(self) -> float | None:
        return _median(self._temperature_from_sources("temperature"))

    @property
    def native_apparent_temperature(self) -> float | None:
        return _median(self._temperature_from_sources("apparent_temperature"))
    ...
    def dew_point(self) -> float | None:
        return _median(self._temperature_from_sources("dew_point"))
